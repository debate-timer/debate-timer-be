...
public class SharingWebSocketController {
    ...
    public SharingResponse share(
        ...
        log.info("웹소켓 이벤트 수신: roomId={}, eventType={}", roomId, request.eventType());
        SharingResponse response = sharingService.share(request);
        log.info("웹소켓 이벤트 발행: roomId={}, eventType={}", roomId, response.eventType());
        return response;
    }
}
